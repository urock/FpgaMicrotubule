...
void run_step_c1(int step);
void run_step_c2(int step);
...

{
	...
	if (load_coords) {
		...
		for (j=0; j<N_d+3; j++)
			for (i=0; i<13; i++)
                         lat_r_x[i][j]=lat_r_y[i][j]=lat_r_t[i][j]=0;

		for (j=0; j<N_d+1; j++)
			for (i=0; i<13; i++){
                         lat_l_x[i][j]=lat_l_y[i][j]=lat_l_t[i][j]=0;
                         long_u_x[i][j]=long_u_y[i][j]=long_u_t[i][j]=0;
                         long_d_x[i][j]=long_d_y[i][j]=long_d_t[i][j]=0;
                   }
...
// update coordinates
	l3: for (j=1; j<N_d; j++)		// coordinates are fixed at j=0
		l4: for (i=0; i<13; i++) {
#pragma HLS PIPELINE
...
//        printf("%d,%d:%.2f,%.2f,%.2f\t%.2f,%.2f,%.2f\t%.2f,%.2f,%.2f\n", i1,j1,x_1,y_1,teta_1, x_2,y_2,teta_2, x_3,y_3,teta_3);
