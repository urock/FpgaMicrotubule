...
{		
	...
	public:
		...
		__device__ void RandomInit(unsigned long s) {
		...
		__device__ float Normal_dist(void);

		__device__ double genrand_real2(void);


//		__device__ void init_genrand(unsigned long s);
...
__device__ float Random :: Normal_dist(void)
...
__device__ double Random :: genrand_real2(void)
...
__device__ void Random :: init_genrand(unsigned long s)
...
mt_cuda_kernel1(
const int          niters,                                                                   
float    x_inout[][13][N_d+3],                          
float    y_inout[][13][N_d+3],
float    t_inout[][13][N_d+3],
unsigned long rs
)
{
...
__shared__ Random rnd[rnd_N];
if (j==0 && i<rnd_N){
 rnd[i].RandomInit(rs+i);
}


if(i==0 && j==0){
// printf("g%d %d\n",g,blockIdx.x + gridDim.x*blockIdx.y);
}
...
__lat_r[i][j]=make_float4(0,0,0,0);
__long_d[i][j]=make_float4(0,0,0,0);
__x[i][j]=x_inout[g][i][j];     
__y[i][j]=y_inout[g][i][j];     
__t[i][j]=t_inout[g][i][j];
...
// t_ij -= dt_viscPF_teta * f_t;
 ...
 x_ij -= dt_viscPF * f_x +  sqrt_PF_xy*rnd[i%rnd_N].Normal_dist();
 y_ij -= dt_viscPF * f_y +  sqrt_PF_xy*rnd[i%rnd_N].Normal_dist();
 t_ij -= dt_viscPF_teta * f_t + sqrt_PF_teta*rnd[i%rnd_N].Normal_dist();
...
if (j!=0){
 x_inout[g][i][j] = __x[i][j];
 y_inout[g][i][j] = __y[i][j];
 t_inout[g][i][j] = __t[i][j];
}
...
#define N_g_Max 1024*16


float x_1[N_g_Max][13][N_d];
float y_1[N_g_Max][13][N_d];
float t_1[N_g_Max][13][N_d];


float x_3[N_g_Max][13][N_d+3];
float y_3[N_g_Max][13][N_d+3];
float t_3[N_g_Max][13][N_d+3];
...
main(int argc, char **argv)
{
    ...
    if(argc>1)N_g_x=atoi(argv[1]);
    N_g=N_g_x*N_g_y;
 
    srand((int)time(NULL)); 

    unsigned long rseed=rand();
  
  size_t size = N_g*13*(N_d+3)*sizeof(float);
    printf("N_g=%d,size=%d\n",N_g,size);
...
*/
        ...
        for(g=0;g<N_g;g++){
	 init_coords(x_1[g],y_1[g],t_1[g]);
	 rnd_coords(x_1[g],y_1[g],t_1[g]);
         cpy_coords2(x_1[g],y_1[g],t_1[g],x_3[g],y_3[g],t_3[g]);
        }
    ...
    dim3 grid_size;
    grid_size.x = N_g_x;//num_elements_x / block_size.x;
    grid_size.y = N_g_y;//num_elements_y / block_size.y;


    printf("CUDA kernel launch with %d,%d %d,%d\n", block_size.x, block_size.y, grid_size.x,grid_size.y);


    boom::mt_cuda_kernel1<<<grid_size, block_size>>>(niters, 
	(float(*)[13][N_d+3])d_x,  
	(float(*)[13][N_d+3])d_y,  
	(float(*)[13][N_d+3])d_t,
	rseed);
...
if(use_cmp){
        for(g=0;g<N_g;g++){
	mt_cpu(niters,1,x_1[g],y_1[g],t_1[g],x_1[g],y_1[g],t_1[g]);

	if(!compare2(x_1[g], x_3[g], &err1, &delta1, epsilon) || 
	   !compare2(y_1[g], y_3[g], &err1, &delta1, epsilon) ||
	   !compare2(t_1[g], t_3[g], &err1, &delta1, epsilon))
        {
