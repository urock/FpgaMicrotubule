...

{
   ...
   if (allocate == 0) {
      ...
      for (int p = 0; p < 13; p++){
         ...
         y[p].resize(N_d+3);
         t[p].resize(N_d+3);
         lat_l_x[p].resize(N_d+1);
         lat_l_y[p].resize(N_d+1);
         lat_l_t[p].resize(N_d+1);
         lat_r_x[p].resize(N_d+3);
         lat_r_y[p].resize(N_d+3);
         lat_r_t[p].resize(N_d+3);
         long_u_x[p].resize(N_d+1);
         long_u_y[p].resize(N_d+1);
         long_u_t[p].resize(N_d+1);
         long_d_x[p].resize(N_d+1);
