...
namespace microtubule {
  ...
  void mt::init(string json_name) {
    ...
    if (use_coeffs_from_json) {
      ...
      printf("Used coeeficients from json\n");
	...
	std::cout << "End of init_coords_and_type" << std::endl;

    // init generator for kinetics
    if (const_seeds) {
      KineticRand = new UniRandom(12345);
      std::cout << "UniRandom" << std::endl;
    } else {
      gettimeofday(&tt1, 0);
  ...
  void mt::init_coords_and_type() {
  	std::cout << "init_coords_and_type" << std::endl;
    unsigned int i,j;
    for (i=0; i<13; i++) {
...
//       return Fpga->CalcDynamics(dev,dynamic_steps, N_d_calc, coords.x, coords.y, coords.y, type_mol);
    ...
    return calc_dynamics_cpu(N_d); 
  }

void mt::pry() {
// 	for (int jg = 0; jg < 24; jg++){
//     	std::cout << "yd["<<jg<<"] -> " << coords.x[0][jg] << std::endl;	
// 	}
