...
int run_step_c(mt_coords_t  &mt_coords, vector<vector<int> > type_mol, bool flag_rand_c, int N_d)
{
	unsigned int i, j;
	...
	for (j = N_d_choose; j < N_d; j++) {
		for (i=0; i<13; i++) {
			mt_coords.x[i ][j] = 0.0f;  
			mt_coords.y[i][j] = -100.0f;
			mt_coords.t[i][j] = 0.0f;
		}
	}
...
int mt::calc_dynamics_cpu(int N_d1) {
	...
	for (int p = 0; p < 13; p++) {
		if (N_d_calc < NStop[p] - NStart[p]){
			N_d_calc = NStop[p] - NStart[p];
		} 
	}
	...
	for (step = 1; step <= dynamic_steps; step++) {
		if (run_step_c(coords, type_mol, brownian_en, N_d1)<0) 
			return -1;	
	}
