...
namespace microtubule {
  ...
  std::cout << "adding dimer i = " << i <<" "<<2*Rad*sinf(coords.t[i][NStop[i] - 1])<< std::endl;
  
     if (NStop[i] > N_d-1) return;
     //todo angle
     coords.x[i][NStop[i]] = coords.x[i][NStop[i] - 1] - 2*Rad*sinf(coords.t[i][NStop[i] - 1]);//?
     std::cout <<coords.x[i][NStop[i] - 1]<<" "<<coords.x[i][NStop[i]]<<std::endl;
...
//              printf("deattached :%i %i r = %f\n", i, j,getDistance(coords.x[i][j+1], coords.y[i][j+1], coords.t[i][j+1], 
//                           coords.x[i][j],   coords.y[i][j],   coords.t[i][j]));
//                           
//             for (int jg = 0; jg < 24; jg++){
//         		std::cout << "y["<<jg<<"] -> " << coords.y[i][jg] << std::endl;
//         	}
...
//     }
	...
	// for (int jg = 0; jg < 24; jg++){
//     	std::cout << "x["<<jg<<"] -> " << coords.x[0][jg] << std::endl;	
// 	}
//	for (int jg = 0; jg < 13/*24*/; jg++){
// 		std::cout << "N["<<jg<<"] -> " << NStop[jg] << std::endl;	
// 	}
// 	std::cout << "y["<<777<<"] -> " << coords.y[7][NStop[7]-1] << std::endl;
	// if (k == 1 || k == 1)
// 	addDimer(0);
